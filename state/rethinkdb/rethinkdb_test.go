...
func TestRethinkDBStateStore(t *testing.T) {
	...
	t.Run("With struct data", func(t *testing.T) {
		...
		if err := db.Set(ctx, &state.SetRequest{Key: k, Value: d}); err != nil {
			t.Fatalf("error setting data to db: %v", err)
		}

		// get set data and compare
		resp, err := db.Get(ctx, &state.GetRequest{Key: k})
		assert.Nil(t, err)
		...
		if err = db.Set(ctx, &state.SetRequest{Key: k, Value: d2, ETag: &tag}); err != nil {
			t.Fatalf("error setting data to db: %v", err)
		}

		// get updated data and compare
		resp2, err := db.Get(ctx, &state.GetRequest{Key: k})
		assert.Nil(t, err)
		...
		// delete data
		if err := db.Delete(ctx, &state.DeleteRequest{Key: k}); err != nil {
...
func testBulk(t *testing.T, db *RethinkDB, i int) {
	...
	ctx := context.Background()
	// bulk set it
	if err := db.BulkSet(ctx, setList); err != nil {
		t.Fatalf("error setting data to db: %v -- run %d", err, i)
	}

	// check for the data
	for _, v := range deleteList {
		resp, err := db.Get(ctx, &state.GetRequest{Key: v.Key})
	...
	// delete data
	if err := db.BulkDelete(ctx, deleteList); err != nil {
...
func TestRethinkDBStateStoreMulti(t *testing.T) {
	...
	t.Run("With multi", func(t *testing.T) {
		...
		if err := db.BulkSet(ctx, list); err != nil {
		...
		// execute multi
		if err := db.Multi(ctx, req); err != nil {
			t.Fatalf("error setting multi to db: %v", err)
		}

		// the one not deleted should be still there
		m1, err := db.Get(ctx, &state.GetRequest{Key: fmt.Sprintf(recordIDFormat, 1)})
		assert.Nil(t, err)
		assert.NotNil(t, m1)
		...
		m2, err := db.Get(ctx, &state.GetRequest{Key: fmt.Sprintf(recordIDFormat, 3)})
		assert.Nil(t, err)
		assert.NotNil(t, m2)
