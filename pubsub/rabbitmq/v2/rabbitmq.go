...
// publishSync  Adding support for x-hash-exchange
func (r *rabbitMQ) publishSync(ctx context.Context, req *pubsub.PublishRequest) (rabbitMQChannelBroker, int, error) {
	...
	exc := req.Topic
	if len(strings.TrimSpace(r.metadata.ExchangeName)) > 0 {
		exc = r.metadata.ExchangeName
	}

	if err := r.ensureExchangeDeclared(r.channel, exc, r.metadata.ExchangeKind, r.metadata.Durable, r.metadata.DeleteWhenUnused); err != nil {
		r.logger.Errorf("%s publishing to %s failed in ensureExchangeDeclared: %v", logMessagePrefix, exc, err)
	...
	//TODO: Eye here
	if r.metadata.ExchangeKind == xConsistentHashExchange {
		if len(strings.TrimSpace(r.metadata.ColumnRouting)) > 0 {
			arg = amqp.Table{r.metadata.ColumnRouting: "IB001"}
		} else {
			r.logger.Errorf("The Exchange: %s does not contain routing id to assign hash header")
		}
	}
	...
	confirm, err := r.channel.PublishWithDeferredConfirmWithContext(ctx, req.Topic, r.metadata.RoutingKey, false, false, p)
	if err != nil {
...
func (r *rabbitMQ) prepareSubscription(channel rabbitMQChannelBroker, req pubsub.SubscribeRequest, queueName string) (*amqp.Queue, error) {
	...
	err := r.ensureExchangeDeclared(channel, exc, r.metadata.ExchangeKind, r.metadata.Durable, r.metadata.DeleteWhenUnused)
	if err != nil {
	...
	if r.metadata.EnableDeadLetter {
		...
		dlxName := fmt.Sprintf(defaultDeadLetterExchangeFormat, exc)
		...
		for key, value := range r.metadata.DeadLetterQueueMap {
			// check if the value is a float
			if floatValue, ok := value.(float64); ok {
				// if so, convert it to an integer and assign that value to dlqArgs
				dlqArgs[key] = int(floatValue)
			} else {
				// if not a float, assign the value as is
				dlqArgs[key] = value
			}
		}
		...
		err = channel.QueueBind(q.Name, r.metadata.RoutingKey, dlxName, false, nil)
		if err != nil {
	...
	for key, value := range r.metadata.QueueMap {
		// check if the value is a float
		if floatValue, ok := value.(float64); ok {
			// if so, convert it to an integer and assign that value to dlqArgs
			args[key] = int(floatValue)
		} else {
			// if not a float, assign the value as is
			args[key] = value
		}
	...
	routingKeys := strings.Split(r.metadata.RoutingKey, ",")
	for i := 0; i < len(routingKeys); i++ {
		routingKey := routingKeys[i]
		r.logger.Debugf("%s binding queue '%s' to exchange '%s' with routing key '%s'", logMessagePrefix, q.Name, exc, routingKey)
		err = channel.QueueBind(q.Name, routingKey, exc, false, nil)
		if err != nil {
...
// this function call should be wrapped by channelMutex. Adding support for x-hash-exchange
func (r *rabbitMQ) ensureExchangeDeclared(channel rabbitMQChannelBroker, exchange, exchangeKind string, durable bool, autoDelete bool) error {
	...
	if !r.containsExchange(exchange) {
		...
		if r.metadata.ExchangeKind == xConsistentHashExchange {
			if len(strings.TrimSpace(r.metadata.ColumnRouting)) > 0 {
				arg = amqp.Table{"hash-header": r.metadata.ColumnRouting}
			} else {
				r.logger.Errorf("The Exchange: %s does not contain routing id to assign hash header", exchange)
			}
		}

		err := channel.ExchangeDeclare(exchange, exchangeKind, durable, autoDelete, false, false, arg)
		if err != nil {
