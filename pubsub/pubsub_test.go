...
func TestBulkPublish_DefaultBulkMessager(t *testing.T) {
	...
	tcs := []struct {
		name                    string
		bulkPublishKeepOrderKey string
	}{
		{
			name:                    "bulkPublishKeepOrderKey is set to true",
			bulkPublishKeepOrderKey: "true",
		},
		{
			name:                    "bulkPublishKeepOrderKey is not true",
			bulkPublishKeepOrderKey: "false",
		},
	}

	for _, tc := range tcs {
		t.Run(fmt.Sprintf("publishes all messages in a request, %s", tc.name), func(t *testing.T) {
			req.Metadata[bulkPublishKeepOrderKey] = tc.bulkPublishKeepOrderKey
