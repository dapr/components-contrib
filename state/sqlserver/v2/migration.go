...
func (m *migration) newMigrationResult() migrationResult {
	r := migrationResult{
		...
		getCommand:               fmt.Sprintf("SELECT [Data], [BinaryData], [isBinary], [RowVersion], [ExpireDate] FROM [%s].[%s] WHERE [Key] = @Key AND ([ExpireDate] IS NULL OR [ExpireDate] > GETDATE())", m.metadata.SchemaName, m.metadata.TableName),
					...
					BEGIN
						IF (@RowVersion IS NOT NULL)
							BEGIN
								...
								BEGIN
									UPDATE [%[3]s]
									SET [Data]=@Data, [isBinary]=@isBinary, [BinaryData]=@BinaryData, UpdateDate=GETDATE(), ExpireDate=CASE WHEN @TTL IS NULL THEN NULL ELSE DATEADD(SECOND, @TTL, GETDATE()) END
						...
						ELSE
							BEGIN
								...
								BEGIN
									BEGIN TRY
										INSERT INTO [%[3]s] ([Key], [Data], [isBinary], [BinaryData], ExpireDate) VALUES (@Key, @Data, @isBinary, @BinaryData, CASE WHEN @TTL IS NULL THEN NULL ELSE DATEADD(SECOND, @TTL, GETDATE()) END)
									END TRY
