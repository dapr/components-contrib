...
const (
	// MSSQL doesn't accept RFC3339 formatted time, rejects trailing 'Z' for UTC indicator.
	MSSQLDateTimeFormat = "2006-01-02 15:04:05"
...
// 4. `export MSSQL_TEST_CONN_URL=daprtest@tcp(localhost:3306)/daprtest`
// 5. `go test -v -count=1 ./bindings/MSSQL -run ^TestMSSQLIntegrationWithURL`

func TestMSSQLIntegration(t *testing.T) {
	url := os.Getenv("MSSQL_TEST_CONN_URL")
	if url == "" {
		t.SkipNow()
	}

	b := NewMSSQL(logger.NewLogger("test"))
	...
	t.Run("Invoke insert", func(t *testing.T) {
		req.Operation = execOperation
		for i := 0; i < 10; i++ {
			req.Metadata[commandSQLKey] = fmt.Sprintf(testInsert, i, i, true, time.Now().Format(MSSQLDateTimeFormat), "{\"key\":\"val\"}")
			res, err := b.Invoke(context.TODO(), req)
			assertResponse(t, res, err)
		}
	})
	...
	t.Run("Invoke update", func(t *testing.T) {
		req.Operation = execOperation
		for i := 0; i < 10; i++ {
			req.Metadata[commandSQLKey] = fmt.Sprintf(testUpdate, time.Now().Format(MSSQLDateTimeFormat), i)
			res, err := b.Invoke(context.TODO(), req)
			assertResponse(t, res, err)
		}
	})
