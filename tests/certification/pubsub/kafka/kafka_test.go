...
// DEBUG with PKCS8
func TestKafkaAuth2(t *testing.T) {
	consumerGroupOIDCCerts := watcher.NewOrdered()
	consumerGroupOIDCSecretKey := watcher.NewOrdered()
	consumerGroupOIDCCertsPKCS8 := watcher.NewOrdered() // Add this
	...
	// PKCS1 format (original)
	keyPEM := pem.EncodeToMemory(&pem.Block{Type: "RSA PRIVATE KEY", Bytes: x509.MarshalPKCS1PrivateKey(key)})

	// PKCS8 format (new)
	keyPKCS8, err := x509.MarshalPKCS8PrivateKey(key)
	require.NoError(t, err)
	keyPEMPKCS8 := pem.EncodeToMemory(&pem.Block{Type: "PRIVATE KEY", Bytes: keyPKCS8})

	os.Setenv("OIDC_CLIENT_ASSERTION_CERT", string(certPEM))
	os.Setenv("OIDC_CLIENT_ASSERTION_KEY", string(keyPEM))
	os.Setenv("OIDC_CLIENT_ASSERTION_CERT_ONELINE", strings.ReplaceAll(string(certPEM), "\n", "\\n"))
	os.Setenv("OIDC_CLIENT_ASSERTION_KEY_PKCS8", string(keyPEMPKCS8)) // Add this
	...
	flow.New(t, "kafka authentication").
		...
		Step("stop sidecar", sidecar.Stop(sidecarNameOIDCCerts)).
		Step("stop app", app.Stop(appIDOIDCCerts)).
		// OIDC with certificates (PKCS8) - NEW
		Step(app.Run(appIDOIDCCerts, fmt.Sprintf(":%d", appPort),
			application(appID1, consumerGroupOIDCCertsPKCS8))).
		Step(sidecar.Run(sidecarNameOIDCCerts,
			append(componentRuntimeOptions(),
				embedded.WithResourcesPath("./components/auth_oidc_certs_pkcs8"),
				embedded.WithAppProtocol(protocol.HTTPProtocol, strconv.Itoa(appPort)),
				embedded.WithDaprGRPCPort(strconv.Itoa(runtime.DefaultDaprAPIGRPCPort)),
				embedded.WithDaprHTTPPort(strconv.Itoa(runtime.DefaultDaprHTTPPort)),
			)...,
		...
		Step("wait", flow.Sleep(10*time.Second)).
		Step("send and wait(in-order)", sendTest(sidecarNameOIDCCerts, consumerGroupOIDCCertsPKCS8)).
		Step("stop sidecar", sidecar.Stop(sidecarNameOIDCCerts)).
		Step("stop app", app.Stop(appIDOIDCCerts)).
		Run()
}
