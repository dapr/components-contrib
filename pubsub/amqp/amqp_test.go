...
func TestSendWithRetry(t *testing.T) {
	...
	t.Run("a component that closes mid-flight stops the retries", func(t *testing.T) {
		a := newComponent(time.Hour)
		sends := 0
		...
		require.Error(t, err)
		assert.Equal(t, 1, sends, "a closed component must not wait out a retry interval")
	})
}
