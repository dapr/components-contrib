...
var (
	...
	mockStaticSecretJSONItemResponse = akeylesssdk.Item{
...
func mockAuthenticate(metadata *akeylessMetadata, akeylessSecretStore *akeylessSecretStore) error {
	...
	out, httpResponse, err := akeylessSecretStore.v2.Auth(context.TODO()).Body(*authRequest).Execute()
...
func TestMain(m *testing.M) {
	// Setup mock server that returns an *akeylesssdk.AuthOutput
	mockGateway = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/get-rotated-secret-value":
			...
			w.Write(jsonResponse)
		case "/list-items":
			listItemsResponse := akeylesssdk.NewListItemsInPathOutput()
			listItemsResponse.SetItems(
				[]akeylesssdk.Item{mockDescribeStaticSecretItemResponse},
			)
...
func TestGetSingleSecretJSON(t *testing.T) {
	gateway := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/get-secret-value":
			...
			w.Write(jsonResponse)
		case "/describe-item":
			mockDescribeItemResponse := akeylesssdk.Item{
				ItemName: &mockStaticSecretJSONName,
				ItemType: &mockDescribeStaticSecretType,
			}
...
func TestGetBulkSecretValues(t *testing.T) {
	gateway := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/list-items":
			items := akeylesssdk.NewListItemsInPathOutput()
			items.SetItems(
				[]akeylesssdk.Item{
	...
	assert.JSONEq(t, expectedDynamicValue, response.Data[dynamicSecretKey][dynamicSecretKey])
	...
	assert.JSONEq(t, "{\"value\":{\"application_id\":\"1234567890\",\"password\":\"r3vE4L3D\",\"username\":\"abcdefghijklmnopqrstuvwxyz\"}}", response.Data[rotatedSecretKey][rotatedSecretKey])
...
func TestGetBulkSecretValuesFromDifferentPaths(t *testing.T) {
	...
	mixedDynamicSecret := "/path/to/mixed/secrets/mixed-dynamic" //nolint:gosec // G101: test data only
	mixedRotatedSecret := "/path/to/mixed/secrets/mixed-rotated" //nolint:gosec // G101: test data only

	// Create mock items for different paths
	staticItem1 := akeylesssdk.Item{
		ItemName:  &staticSecret1,
		ItemType:  &mockDescribeStaticSecretType,
		IsEnabled: func(b bool) *bool { return &b }(true),
	}
	staticItem2 := akeylesssdk.Item{
		ItemName:  &staticSecret2,
		ItemType:  &mockDescribeStaticSecretType,
		IsEnabled: func(b bool) *bool { return &b }(true),
	}
	staticItem3 := akeylesssdk.Item{
		ItemName:  &staticSecret3,
		ItemType:  &mockDescribeStaticSecretType,
		IsEnabled: func(b bool) *bool { return &b }(true),
	}
	dynamicItem1 := akeylesssdk.Item{
		ItemName:  &dynamicSecret1,
		ItemType:  &mockDescribeDynamicSecretType,
		IsEnabled: func(b bool) *bool { return &b }(true),
	}
	dynamicItem2 := akeylesssdk.Item{
		ItemName:  &dynamicSecret2,
		ItemType:  &mockDescribeDynamicSecretType,
		IsEnabled: func(b bool) *bool { return &b }(true),
	}
	rotatedItem1 := akeylesssdk.Item{
		ItemName:  &rotatedSecret1,
		ItemType:  &mockDescribeRotatedSecretType,
		IsEnabled: func(b bool) *bool { return &b }(true),
	}
	mixedStaticItem := akeylesssdk.Item{
		ItemName:  &mixedStaticSecret,
		ItemType:  &mockDescribeStaticSecretType,
		IsEnabled: func(b bool) *bool { return &b }(true),
	}
	mixedDynamicItem := akeylesssdk.Item{
		ItemName:  &mixedDynamicSecret,
		ItemType:  &mockDescribeDynamicSecretType,
		IsEnabled: func(b bool) *bool { return &b }(true),
	}
	mixedRotatedItem := akeylesssdk.Item{
	...
	gateway := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/list-items":
			...
			var listItemsRequest akeylesssdk.ListItems
			...
			var items akeylesssdk.ListItemsInPathOutput
			...
			case "/":
				...
				items.SetItems([]akeylesssdk.Item{})

			case "/path/to/static/secrets":
				// Static secrets folder
				items.SetItems([]akeylesssdk.Item{staticItem1, staticItem2, staticItem3})
				items.SetFolders([]string{})

			case "/path/to/dynamic/secrets":
				// Dynamic secrets folder
				items.SetItems([]akeylesssdk.Item{dynamicItem1, dynamicItem2})
				items.SetFolders([]string{})

			case "/path/to/rotated/secrets":
				// Rotated secrets folder
				items.SetItems([]akeylesssdk.Item{rotatedItem1})
				items.SetFolders([]string{})

			case "/path/to/mixed/secrets":
				// Mixed secrets folder
				items.SetItems([]akeylesssdk.Item{mixedStaticItem, mixedDynamicItem, mixedRotatedItem})
				items.SetFolders([]string{})
		...
		case "/describe-item":
			...
			var describeItemRequest akeylesssdk.DescribeItem
	...
	assert.JSONEq(t, expectedDynamicValue1, response.Data[dynamicSecret1][dynamicSecret1])
	...
	assert.JSONEq(t, expectedDynamicValue2, response.Data[dynamicSecret2][dynamicSecret2])
	...
	assert.JSONEq(t, expectedRotatedValue1, response.Data[rotatedSecret1][rotatedSecret1])
	...
	assert.JSONEq(t, expectedMixedDynamicValue, response.Data[mixedDynamicSecret][mixedDynamicSecret])
