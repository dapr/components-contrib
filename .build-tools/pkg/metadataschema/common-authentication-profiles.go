...
// ParseCommonAuthenticationProfile returns an AuthenticationProfile(s) from a given CommonAuthenticationProfile.
func ParseCommonAuthenticationProfile(bi CommonAuthenticationProfile) ([]AuthenticationProfile, error) {
	switch bi.Name {
	case "aws":
	...
	case "gcp": 
	   ...
	   return nil, nil
	default:
		return nil, fmt.Errorf("common authentication profile %s does not exist", bi.Name)
	}
}