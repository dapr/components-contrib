...
// To ensure this doesn't happen, when we create a new subscription to the same topic and with the same consumer group, we check if there's a file in Azure Storage with the checkpoint created by the old SDK and with a still-active lease. If that's true, we wait up to 10 minutes before we crash Dapr with a log message describing what's happening.
...
func (aeh *AzureEventHubs) ensureNoTrack1Subscribers(parentCtx context.Context, topic string) error {
	...
	for pager.More() {
		...
		cancel()
		if err != nil {
			fmt.Errorf("failed to list blobs: %w", err)
		}
