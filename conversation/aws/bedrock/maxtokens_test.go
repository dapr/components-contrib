...
func TestInitWiresMaxTokens(t *testing.T) {
	b := NewAWSBedrock(logger.NewLogger("test")).(*AWSBedrock)
	err := b.Init(t.Context(), conversation.Metadata{
		Base: metadata.Base{Properties: map[string]string{"region": "us-east-1", "accessKey": "test-key", "secretKey": "test-secret", "model": "amazon.titan-text-lite-v1", "maxTokens": "50"}},
	})
	if err != nil {
	...
	_, err = b.Converse(t.Context(), &conversation.Request{
