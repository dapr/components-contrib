...
func InitTransactionStateStore(metadata Metadata) (error, *redis.UniversalClient) {
	// 1. parse config
	...
	if metadata.Properties["redisHost"] == "" {
		return fmt.Errorf("InitTransactionstateStore error: redisHost is empty"), nil
	}
	...
	if _, err = client.Ping(ctx).Result(); err != nil {
		return fmt.Errorf("InitTransactionstateStore error connecting to redis at %s: %s", clientSettings.Host, err), nil
	}
	return nil, &client
}
